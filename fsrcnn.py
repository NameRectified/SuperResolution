...
import tensorflow as tf
from tensorflow.keras.layers import Input, Conv2D, PReLU
...
def fsrcnn(d, s, m, channels):
    inputs = Input(shape=(None, None, 3))  # Assuming RGB images
    ...
    # Layer 5: Deconvolution (upsampling) using PyTorch
    # We will implement the upsampling in the `upscale_image` function
    return Model(inputs=inputs, outputs=x)
...
def upscale_image(model, lr_image_path, scale_factor=2):
    ...
    # Convert to PyTorch tensor for deconvolution
    sr_image = torch.tensor(sr_image, dtype=torch.float32)

    # Deconvolution (upsampling) using PyTorch
    # The input channels (d) should match the output of the last Conv2D layer in TensorFlow
    deconv_layer = torch.nn.ConvTranspose2d(in_channels=sr_image.shape[1], out_channels=3, kernel_size=9, stride=2, padding=4)

    # Up-sample the image
    sr_image = deconv_layer(sr_image)

    # Process output to match image format
    sr_image = sr_image.detach().numpy()
    sr_image = np.squeeze(sr_image, axis=0)
    sr_image = np.clip(sr_image, 0, 1)
    sr_image = (sr_image * 255).astype(np.uint8)
...
def main():
    ...
    if os.path.exists(model_path):
        ...
        lr_images, hr_images = load_dataset(image_dir, scale_factor)
        model = fsrcnn(d=56, s=12, m=4, channels=3)  # Initialize FSRCNN with your parameters
        train_model(model, lr_images, hr_images, batch_size=batch_size, epochs=epochs)

    test_image_path = r'testImages/image3.png'
    sr_image = upscale_image(model, test_image_path)
    cv2.imwrite('fsrcnn_test_image.jpg', sr_image)
